...
for (const [code, item] of Object.entries(locales)) {
  ...
  const links = [
    `            <a href="${homePath(item)}custom-spelling-words-game">${footerLinks[code][0]}</a>`,
    `            <a href="${homePath(item)}weekly-spelling-practice">${footerLinks[code][1]}</a>`,
    `            <a href="${homePath(item)}sight-word-typing-game">${footerLinks[code][2]}</a>`,

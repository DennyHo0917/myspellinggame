...
for (const [code, item] of Object.entries(locales)) {
  ...
  const links = [
    ...
    `            <a href="${homePath(item)}spelling-practice-for-parents">${footerLinks[code][1]}</a>`,
    `            <a href="${homePath(item)}custom-spelling-words-game">${footerLinks[code][2]}</a>`,
    `            <a href="${homePath(item)}spelling-assignments-for-teachers">${footerLinks[code][3]}</a>`,
    `            <a href="${homePath(item)}weekly-spelling-practice">${footerLinks[code][4]}</a>`,
    `            <a href="${pagePath(item)}">${footerLinks[code][5]}</a>`,
    `            <a href="${homePath(item)}privacy">${footerLinks[code][6]}</a>`,
    `            <a href="${homePath(item)}about">${footerLinks[code][7]}</a>`,
    `            <a href="${homePath(item)}contact">${footerLinks[code][8]}</a>`,
  ].join(" &middot; ");

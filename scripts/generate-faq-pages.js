...
for (const [code, item] of Object.entries(locales)) {
  ...
  const links = [
    ...
    `            <a href="${pagePath(item)}">${footerLinks[code][4]}</a>`,
    `            <a href="${homePath(item)}privacy">${footerLinks[code][5]}</a>`,
    `            <a href="${homePath(item)}about">${footerLinks[code][6]}</a>`,
    `            <a href="${homePath(item)}contact">${footerLinks[code][7]}</a>`,
  ].join(" &middot; ");

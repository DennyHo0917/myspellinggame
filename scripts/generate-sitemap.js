...
const gitAvailable = gitOutput(['rev-parse', '--is-inside-work-tree']) === 'true';
const gitShallow = gitAvailable && gitOutput(['rev-parse', '--is-shallow-repository']) === 'true';
...
function lastModified(file) {
  ...
  if (gitAvailable && !gitShallow) {
    const value = gitOutput(['log', '-1', '--format=%cs', '--', relative]);
    if (validDate(value)) return value;
  }
  if (gitShallow) return null;
  try {
    const value = fs.statSync(file).mtime.toISOString().slice(0, 10);
    if (validDate(value)) return value;
  } catch {}
  return null;
}
